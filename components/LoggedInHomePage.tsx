...
const LoggedInHomePage: React.FC<LoggedInHomePageProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<"explore" | "dashboard">(
    "explore"
  );
      ...
      if (donationsData) {
        ...
        if (userSpecificDonations.length > 0) {
          const earliestDate = userSpecificDonations.reduce(
            (earliest, d) =>
              new Date(d.date) < new Date(earliest) ? d.date : earliest,
  ...
  useEffect(() => {
    ...
    const loadCurrentUser = () => {
      const storedUser = localStorage.getItem("donateTransparentlyCurrentUser");
      if (storedUser) {
        try {
  ...
  const charities: Charity[] = [
    ...
    },
    {
      name: "Oxfam",
      description:
        "Global movement to end the injustice of poverty through humanitarian aid, advocacy, development.",
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-purple-600">
                {charities.reduce(
                  (total, charity) => total + charity.organizations.length,
                  8
                )}
              </p>
            </div>
      ...
      <div>
        <div className="flex justify-between items-center mb-6">
          ...
          <p className="text-sm text-gray-500">
            {charities.length} charities • Real-time donation tracking
          </p>
        </div>
  ...
  return (
    <div className="container mx-auto pt-24 px-8 min-h-screen pb-24">
      ...
      {activeTab === "explore" ? (
        ...
        <DashboardTab userEmail={userEmail} />
      )}
    </div>
  );
};
